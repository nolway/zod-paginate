 ...
 */
type Path<T, D extends number = 5> = D extends 0
  ? never
  : T extends Primitive
    ? never
 ...
 */
function pickFromAllowlist<TSchema extends DataSchema>(
...
function expandSelect<TSchema extends DataSchema>(
...
function computeSelect<TSchema extends DataSchema>(
...
function getOwnProp(obj: Record<string, unknown>, key: string): unknown {
...
function getZodAtPath(obj: z.ZodObject<z.ZodRawShape>, path: string): z.ZodType {
    ...
    .pipe(
      baseSchema
        ...
        .transform((val): PaginationQueryParams<TSchema> => {
          ...
          const filters = buildWhereAstWithGroups(val.rawFilters, val.groupDefs);
